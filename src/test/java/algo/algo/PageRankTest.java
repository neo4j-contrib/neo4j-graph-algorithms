...
import algo.algo.pagerank.PageRankAlgoTest;
...
public class PageRankTest
{
    ...
    public void shouldGetPageRankExpectedResult() throws IOException
    {
        ...
        String query = "MATCH (b:Company {name:'b'})\n" +
                       "CALL algo.algo.pageRankWithConfig([b],{iterations:20,types:'TYPE_1|TYPE_2'}) " +
                       "YIELD node, score\n" +
    ...
    public void shouldGetPageRankStats() throws IOException
    {
        db.execute( COMPANIES_QUERY ).close();
        String query =
                       "CALL algo.algo.pageRankStats({iterations:20}) ";
        Result result = db.execute( query );
        assertTrue( result.hasNext() );
        Map<String,Object> row = result.next();
    ...
    {
        db.execute( COMPANIES_QUERY ).close();
        Result result = db.execute("CALL algo.algo.pageRankWithCypher({iterations:20, write:true}) ");
    ...
    public void shouldGetPageRankWithCypherExpectedResultWithLables() throws IOException
    {
        ...
        Result result = db.execute("CALL algo.algo.pageRankWithCypher({iterations:20, write:true, node_cypher:'MATCH (node:Company) return id(node) as id'}) ");
        System.out.println(result.resultAsString());
    ...
    {
        db.execute( COMPANIES_QUERY_LABEL ).close();
        Result result = db.execute("CALL algo.algo.pageRankWithCypher({iterations:20, write:true, node_cypher:'none', rel_cypher:'MATCH (n:Company)-->(o) RETURN id(n) as source, id(o) as target', weight:null}) ");
        System.out.println(result.resultAsString());
        ResourceIterator<Double> it = db.execute("MATCH (n) WHERE exists(n.pagerank) RETURN n.name as name, n.pagerank as score ORDER BY score DESC LIMIT 1").columnAs("score");
        assertTrue( it.hasNext() );
        assertEquals( PageRankAlgoTest.EXPECTED, it.next(), 0.1D );
    ...
    {
        db.execute( COMPANIES_QUERY_LABEL ).close();
        Result result = db.execute("CALL algo.algo.pageRankWithCypher({iterations:20, write:true, node_cypher:'none', " +
    ...
    {
        db.execute( COMPANIES_QUERY_LABEL ).close();
        Result result = db.execute("CALL algo.algo.pageRankWithCypher({write:true, iterations:5, node_cypher:'MATCH (node:Company) return id(node) as id LIMIT 5',rel_cypher:'MATCH (c:Company)-[:TYPE1]->(o) RETURN id(c) as source,id(o) as target',weighted:false}) ");
        System.out.println(result.resultAsString());
        ResourceIterator<Double> it = db.execute("MATCH (n) WHERE exists(n.pagerank) RETURN n.name as name, n.pagerank as score ORDER BY score DESC LIMIT 1").columnAs("score");
        assertTrue( it.hasNext() );
    ...
    public void shouldGetPageRankExpectedResultWithTypes() throws IOException
    {
        db.execute( COMPANIES_QUERY ).close();
        String query = "MATCH (b:Company {name:'b'})\n" +
                       "CALL algo.algo.pageRankWithConfig([b],{iterations:20}) YIELD node, score\n" +
                       "RETURN node.name AS name, score\n" +
                       "ORDER BY score DESC";
        Result result = db.execute( query );
        assertTrue( result.hasNext() );
    ...
    public void shouldHandleEmptyNodeSet()
    {
        ...
        assertExpectedResult( 0, "CALL algo.algo.pageRank([])" + "" );
        assertExpectedResult( 0, "CALL algo.algo.pageRankWithConfig([],{iterations:10})" + "" );

        assertExpectedResult( 0, "CALL algo.algo.pageRank([])" );
        assertExpectedResult( 0, "CALL algo.algo.pageRankWithConfig([],{iterations:10,types:'TYPE'})" );
    }
    ...
    public void shouldHandleNullNodes()
    {
        ...
        assertExpectedResult( 0, algoQuery( "CALL algo.algo.pageRank(null)" ) );
        assertExpectedResult( 0, algoQuery( "CALL algo.algo.pageRankWithConfig(null,{iterations:10})" ) );
    ...
    public void shouldHandleNullIterations()
    {
        ...
        assertExpectedResult( 0, algoQuery( "CALL algo.algo.pageRankWithConfig(nodes,{iterations:null})" ) );
