...
import static algo.util.TestUtil.testCall;
import static algo.util.TestUtil.testResult;
...
public class PathFindingTest {
    ...
    public void testAStar() throws Exception {
        ...
        testResult(db,
                "MATCH (from:City {name:'München'}), (to:City {name:'Hamburg'}) " +
                        "CALL algo.algo.aStar(from, to, 'DIRECT', 'dist', 'lat', 'lon') yield path, weight " +
                        "RETURN path, weight" ,
                r ->  {
    ...
    public void testAStarConfig() throws Exception {
        ...
        testResult(db,
                "MATCH (from:City {name:'München'}), (to:City {name:'Hamburg'}) " +
                        "CALL algo.algo.aStarConfig(from, to, 'DIRECT', {weight:'dist',y:'lat', x:'lon',default:100}) yield path, weight " +
                        "RETURN path, weight" ,
                r ->  {
                    assertEquals(true, r.hasNext());
                    Map<String, Object> row = r.next();
    ...
    public void testDijkstra() {
        ...
        testCall(db,
            ...
            "CALL algo.algo.dijkstra(from, to, 'ROAD>', 'd') yield path, weight " +
            "RETURN path, weight" ,
            row ->  {
        ...
        testCall(db,
            "MATCH (from:Loc{name:'A'}), (to:Loc{name:'D'}) " +
            "CALL algo.algo.dijkstra(from, to, '', 'd') yield path, weight " +
            "RETURN path, weight" ,
            row ->  {
    ...
    public void testDijkstraWithDefaultWeight() {
        ...
        testCall(db,
                "MATCH (from:Loc{name:'A'}), (to:Loc{name:'D'}) " +
                        "CALL algo.algo.dijkstraWithDefaultWeight(from, to, 'ROAD>', 'd', 10.5) yield path, weight " +
                        "RETURN path, weight",
                row -> {
    ...
    public void testAllSimplePaths() {
        ...
        testResult(db,
                "MATCH (from:Loc{name:'A'}), (to:Loc{name:'D'}) " +
                        "CALL algo.algo.allSimplePaths(from, to, 'ROAD>', 3) yield path " +
