...
* for each pair of nodes in the network calculate the length of the shortest path from one to the other (geodesic distance).
* for each node sums the total distance from the node to all other nodes.
...
Our algorithms returns a *normalized closeness centrality* score where nodes with a higher score are more central.
// end::explanation[]
