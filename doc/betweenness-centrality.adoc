...
Currently, the fastest known algorithm for exactly computing betweenness of all the vertices, designed by Brandes, requires at least O(nm) time for unweighted graphs, where n
is the number of vertices and m is the number of edges. 
Thus, for large-scale graphs, exact centrality computation on current workstations is not practically viable.[2] 

Fast betweenness estimation is thus an important problem, as a good approximation is an acceptable alternative to exact scores.
...
* random: nodes are selected uniformly at random with defined probability of selection. Default probability is `log10(N) / e^2`. If probability is 1 algorihm works as original Brandes, where all nodes are loaded.
