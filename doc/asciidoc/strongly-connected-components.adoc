...
We can see that we have 2 strongly connected components in our sample graph.
The first and biggest component has members Alice, Bridget, and Michael, while the second component has Doug and Mark.
Charles doesn't fit into either of these components because there isn't an outgoing relationship from that node to any of the others.
