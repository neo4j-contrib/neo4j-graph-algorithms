...
// tag::introduction[]
The _Strongly Connected Components_ (SCC) algorithm partitions nodes in a directed graph into connected components (groups) where nodes that fall into the same component are reachable from each other in both directions.
...
// tag::explanation[]
SCC is one of the earliest graph algorithms and the first linear-time algorithm was created by Tarjan in 1972.
