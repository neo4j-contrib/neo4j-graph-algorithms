...
// tag::introduction[]
The _Single Source Shortest Path_ algorithm calculates the shortest (weighted) path between a pair of nodes.
...
Dijkstra came up with his algorithm in 1956 while trying to come up with something to show off the new ARMAC computers that people not familiar with computing would be able to understand.
He designed what is now known as Dijkstra's algorithm and later implemented for a slightly simplified transportation map of 64 cities in the Netherlands.
