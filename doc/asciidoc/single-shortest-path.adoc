...
// tag::use-case[]
The single source shortest path algorithm has found uses in several areas, including:
...
For example, when you view someone's profile on LinkedIn it will indicate how many people separate you in the connections graph as well as listing those mutual connections are.
