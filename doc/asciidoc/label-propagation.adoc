...
// tag::constraint[]
In contrast with other algorithms label propagation can result in different community structures.
The range of solutions can be narrowed if some nodes are given preliminary labels while others are unlabelled.
Unlabelled nodes will be more likely to adapt to the labelled ones.
