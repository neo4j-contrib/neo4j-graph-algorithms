...
PageRank is an iterative algorithm and by default our implementation will run for 20 iterations.
Nodes are initialized with a small amount of PageRank.
On every iteration a percentage of that PageRank is disseminated to each of its outgoing neighbours.

Influential nodes will receive these disseminations more frequently and will therefore end up with a higher score.
// end::introduction[]
